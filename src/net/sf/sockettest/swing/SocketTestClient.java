...
public class SocketTestClient extends JPanel {
    ...
    private JButton ipREALButton = new JButton("Obter IP REAl");
    ...
    public SocketTestClient(final JFrame parent) {
        ...
        gbc.gridwidth = 1;
        gbc.fill = GridBagConstraints.NONE;
        ipREALButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                ipField.setText(getIPreal());
            }
        });
        toPanel.add(ipREALButton, gbc);
...
//    FUNÇÃO ADICIONAL PARA OBTER O IP REAL DO COMPUTADOR PARA TESTES AVANÇADOS
//    ADICIONADO POR MIQUÉIAS FERANDES EM 22/10/2016
    public String getIPreal() {
        System.out.println("SCRIPT DESENVOLVIDO POR MIQUÉIAS FERNANDES");
        System.out.println("baixando arquivo");
        try {
            URL website = new URL("http://meuip.com/");
            ReadableByteChannel rbc = Channels.newChannel(website.openStream());
            FileOutputStream fos = new FileOutputStream("meuip.txt");
            fos.getChannel().transferFrom(rbc, 0, Long.MAX_VALUE);

           // InputStream s = Runtime.getRuntime().exec("wget http://meuip.ciasc.gov.br/ -O meuip.txt").getInputStream();
            Scanner sc = new Scanner("meuip.txt");
            while (sc.hasNext()) {
                String next = sc.next();
        ...
        String ip = "erro";
        try {
            s = new Scanner(new FileReader("meuip.txt"));
            while (s.hasNext()) {
                String next = s.next();
            ...
            Logger.getLogger(SocketTestClient.class.getName()).log(Level.SEVERE, null, ex);
        }
